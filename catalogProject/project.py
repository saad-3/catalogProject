...
def showBrnds():
    ...
    if 'username' not in login_session:
        return render_template('publicshowbrands.html', brands = brands , products = products  )
    else:
...
def specificProduct(brand_id , product_id):
    ...
    if brand.id == products.brand_id:
        if 'username' not in login_session or creator.id != login_session['user_id']:
            return render_template('publicspecificproduct.html', brand =brand , product = products , product_id = product_id, brand_id = brand_id )
        else:
            return render_template('specificproduct.html', brand =brand , product = products , product_id = product_id, brand_id = brand_id )
    else:
        return "Error in url " + """<a href='/'>Back to main page </a>"""


@app.route('/techbrands/<int:brand_id>/products/new/' , methods=['GET', 'POST'])
def newProduct(brand_id):
    if 'username' not in login_session:
        return redirect('/login')

    if request.method == 'POST':
        newProduct = Product(name=request.form['name'], description=request.form[
                           'description'], price=request.form['price'] , brand_id = brand_id , user_id =login_session['user_id'])
...
@app.route('/techbrands/<int:brand_id>/products/<int:product_id>/edit/', methods=['GET', 'POST'])
def editProduct(brand_id , product_id):
    ...
    if request.method == 'POST':
        ...
        return render_template('editproduct.html', brand_id = brand_id , product_id = product_id)

@app.route('/techbrands/<int:brand_id>/products/<int:product_id>/delete/', methods=['GET', 'POST'])
def deleteProduct(brand_id,product_id):
    ...
    if request.method == 'POST':
        ...
        return render_template('deleteproduct.html' , brand_id = brand_id , product_id = product_id)
